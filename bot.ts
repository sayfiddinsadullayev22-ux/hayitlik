...
const PLATFORM_URL = process.env.PLATFORM_URL || 'https://ais-dev-pdienhuyy6mj2mwkt3vk3o-31627344499.asia-southeast1.run.app';
...
async function checkSubscription(userId: number) {
  try {
    for (const channel of CHANNELS) {
      try {
        // Handle both usernames and numeric IDs
        const chatIdentifier = (channel.startsWith('-') || /^\d+$/.test(channel))
          ? channel
          : `@${channel.replace(/^@/, '')}`;

        const member = await bot.telegram.getChatMember(chatIdentifier, userId);
        if (['left', 'kicked', 'restricted'].includes(member.status)) {
          // If restricted, they might still be in the chat but can't post, 
          // but for subscription check, 'left' and 'kicked' are the main ones.
          // Some bots also check 'member' vs 'administrator' vs 'creator'.
        ...
        if (error.description && error.description.includes('chat not found')) {
          console.warn(`[Subscription Check] Chat not found: ${channel}. Skipping this check. Ensure the bot is an admin in the channel.`);
          continue; // Skip misconfigured channels instead of blocking users
        }
        if (error.description && error.description.includes('user not found')) {
          return { isSubscribed: false, channel };
        }
        console.error(`[Subscription Check] Error checking ${channel}:`, error.description || error.message);
        // For other errors (like bot not being an admin), we might want to skip or fail.
        // Let's skip to be user-friendly if the bot is misconfigured.
        continue;
      }
    }
    return { isSubscribed: true, channel: null };
  } catch (error) {
    console.error('General subscription check error:', error);
...
function subscriptionKeyboard() {
  const buttons = CHANNELS.map(channel => {
    const isId = channel.startsWith('-') || /^\d+$/.test(channel);
    const url = isId ? `https://t.me/c/${channel.replace(/^-100/, '')}/1` : `https://t.me/${channel.replace(/^@/, '')}`;
